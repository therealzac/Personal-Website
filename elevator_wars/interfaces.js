...
var asElevatorInterface = function(obj, elevator, floorCount, errorHandler) {
    ...
    elevatorInterface.goToFloor = function(floorNum, forceNow) {
        ...
        if (elevatorInterface.destinationQueue.length) {
            var adjacentElement;
            if (forceNow){
               adjacentElement = _.first(elevatorInterface.destinationQueue);
            } else {
              adjacentElement = _.last(elevatorInterface.destinationQueue);
            }
    ...
    elevatorInterface.currentFloor = function() { return elevator.currentFloor + 1; };
    ...
    elevator.on("floor_button_pressed", function(floorNum) {
        tryTrigger("floor_button_pressed", floorNum + 1);
    });
